    ...
    def __init__(self, in_features, out_features, kernel_size, stride = 1, seperable = False) -> None:
        ...
        if seperable is False:
            ...
            self.net = nn.Sequential(
                ...
                nn.Conv2d(in_features, out_features, kernel_size=1, stride=stride, padding=0,),
                nn.BatchNorm2d(out_features),
                nn.LeakyReLU(0.2)
            )
...
class MobileNet(nn.Module):
    def __init__(self, in_channels, num_classes, alpha = 1) -> None:
        ...
        layers.append(ConvBlock(32, 32, 3, 1, seperable=True));
        layers.append(ConvBlock(32, 64, 1, stride=1));
        layers.append(ConvBlock(64, 64, 3, stride=2, seperable=True));
        layers.append(ConvBlock(64, 128, 1, stride=1));
        layers.append(ConvBlock(128, 128, 3, stride=1, seperable=True));
        layers.append(ConvBlock(128, 128, 1, stride=1));
        layers.append(ConvBlock(128, 128, 3, stride=2, seperable=True));
        layers.append(ConvBlock(128, 256, 1, stride=1));
        layers.append(ConvBlock(256, 256, 3, stride=1, seperable=True));
        layers.append(ConvBlock(256, 256, 1, stride=1));
        layers.append(ConvBlock(256, 256, 3, stride=2, seperable=True));
        layers.append(ConvBlock(256, 512, 1, stride=1));
        for i in range(5):
            layers.append(ConvBlock(512, 512, 3, stride=1, seperable=True));
            layers.append(ConvBlock(512, 512, 1, stride=1));
        
        layers.append(ConvBlock(512, 512, 3, stride=2, seperable=True));
        layers.append(ConvBlock(512, 1024, 1, stride=1));
        layers.append(ConvBlock(1024, 1024, 1, stride=2, seperable=True));
